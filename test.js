...
// 定义一个函数，用来从Favicon Grabber获取favicon的URL
function getFaviconURL(url) {
  // 创建一个XMLHttpRequest对象
  var xhr = new XMLHttpRequest();
  // 设置请求的URL，使用Favicon Grabber的API
  xhr.open("GET", "https://favicongrabber.com/api/grab/" + url);
  // 设置响应的类型为JSON
  xhr.responseType = "json";
  // 定义一个回调函数，当请求完成时执行
  xhr.onload = function() {
    // 如果请求成功，且返回的JSON对象有icons属性
    if (xhr.status == 200 && xhr.response.icons) {
      // 从返回的JSON对象中获取第一个icon的URL
      var faviconURL = xhr.response.icons[0].src;
      // 返回favicon的URL
      return faviconURL;
    }
  };
...
function showShortcuts(folder) {
  ...
  for (let shortcut of shortcuts) { // 遍历每个快捷方式
    ...
    shortcutButton.innerHTML = `<img src="${faviconURL}" alt="快捷方式图标">${shortcut.title}`; // 设置快捷方式按钮的内容，包括图标和标题，如果图标无法加载，就隐藏图标
