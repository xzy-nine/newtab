...
export const Notification = {
  ...
  getNotificationConfig: (type) => {
    ...
    if (type === "error") {
      ...
      const errorDuration = Notification.hasDOMEnvironment()
        ? parseInt(localStorage.getItem("notification-duration-error") || "8000")
        : 8000;
      return {
    ...
    const duration = Notification.hasDOMEnvironment()
      ? parseInt(
          localStorage.getItem(`notification-duration-${type}`) || defaultDurations[type] || 3000,
        )
  ...
  notify: (options) => {
    ...
    // 智能判断是否发送到弹出页面
    const shouldSendToPopup =
      sendToPopup && !forceLocal && type !== "loading" && Notification.isExtensionEnvironment();
    ...
    const copyButtonHtml =
      type !== "loading"
        ? '<button class="notification-copy" title="' +
          I18n.getMessage("copyToClipboard", "复制内容") +
          '">' +
    ...
    // 添加到通知管理系统
    Notification.notificationManager.add(
      notification,
      finalDuration,
      closeNotification,
      checkVisibilityAndSetTimeout,
    );
  ...
  adjustNotificationPositions: () => {
    ...
    notifications.forEach((notification, index) => {
      ...
      notification.classList.remove(
        ...
        "notification-offset-2",
        "notification-offset-3",
        "notification-offset-4",
        "notification-offset-5",
      );
    ...
    if (!loadingNotification) {
      ...
      const notification = Notification.notify({
        ...
        message:
          ...
          '<div class="notification-loading-message">' +
          displayText +
          "</div>" +
          "</div>",
        type: "loading",
  ...
  background: {
    ...
    info: (title, message, options) =>
      Notification.background.sendNotification("info", title, message, options),

    /**
     * 显示成功通知
     */
    success: (title, message, options) =>
      Notification.background.sendNotification("success", title, message, options),

    /**
     * 显示警告通知
     */
    warning: (title, message, options) =>
      Notification.background.sendNotification("warning", title, message, options),

    /**
     * 显示错误通知
     */
    error: (title, message, options) =>
      Notification.background.sendNotification("error", title, message, options),
  },
