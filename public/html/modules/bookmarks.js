...
export const BookmarkManager = {
  ...
  applySelectedFolder: async function (root) {
    try {
      ...
      if (selectedFolder) {
        ...
        // 清除所有选中状态
        document
          .querySelectorAll(".folder-button.selected")
    ...
    if (isCurrentlySelected) {
      ...
      if (isPinned) {
        ...
        const regularButton = document.querySelector(
          `[data-folder-id="${folderId}"][data-pinned="false"]`,
        );
        if (regularButton) {
        ...
        const pinnedButton = document.querySelector(
          `[data-folder-id="${folderId}"][data-pinned="true"]`,
        );
        if (pinnedButton) {
  ...
  showShortcuts: async function (folder) {
    ...
    const shortcuts = DesktopSystem.createShortcutsFromBookmarks(
      bookmarks,
      folder.id,
      gridConfig.cols,
    );
    ...
    try {
      ...
      folderWidgets.forEach((widgetData, containerIndex) => {
        ...
        const firstItemType =
          widgetData.items && widgetData.items[0] ? widgetData.items[0].type : "unknown";
    ...
    requestAnimationFrame(async () => {
      ...
      let pageIndex = Number.isFinite(shortcutList._desktopPageIndex)
        ? shortcutList._desktopPageIndex
        : 0;
      if (pageIndex >= pageCount) pageIndex = pageCount - 1;
      shortcutList._desktopPageIndex = pageIndex;

      const pageItems = layoutItems.filter(
        (item) => (Number.isFinite(item.page) ? item.page : 0) === pageIndex,
      );
        ...
        if (!shortcutList._pageWheelHandler) {
          shortcutList._pageWheelHandler = (event) => {
            ...
            const nextIndex = Math.min(
              pageCount - 1,
              Math.max(0, shortcutList._desktopPageIndex + direction),
            );
          ...
          shortcutList.addEventListener("wheel", shortcutList._pageWheelHandler, {
            passive: false,
          });
        }
      }
    ...
    try {
      ...
      for (let folder of folders) {
        const hasSubfolders = folder.children.some(
          (child) => child.children && !this.isFolderEmpty(child),
        );
    ...
    const formItems = [
      {
        ...
        render: (container) => {
          ...
          const previewLabel = Utils.createElement(
            "label",
            "",
            {},
            I18n.getMessage("shortcutIconPreview", "当前图标预览"),
          );
          ...
          const refreshButton = Utils.createElement(
            "button",
            "btn",
            {},
            I18n.getMessage("shortcutIconRefresh", "重新获取图标"),
          );
          ...
          const fileLabel = Utils.createElement(
            "label",
            "",
            {},
            I18n.getMessage("shortcutIconFile", "本地图标（可选）"),
          );
          ...
          const urlLabel = Utils.createElement(
            "label",
            "",
            {},
            I18n.getMessage("shortcutIconUrl", "图标URL（可选）"),
          );
      ...
      const menuItems = [
        {
          ...
          text: isPinned
            ? I18n.getMessage("unpinFolder", "取消固定文件夹")
            : I18n.getMessage("pinFolder", "固定文件夹"),
          callback: () => {
            if (isPinned) {
    ...
    try {
      ...
      for (const folderId of pinnedFolders) {
        ...
        if (folder && !this.isFolderEmpty(folder)) {
          if (!hasPinnedFolders) {
            // 只有在有实际固定文件夹时才添加标题
            const pinnedHeader = Utils.createElement(
              "h3",
              "section-header",
              {},
              I18n.getMessage("pinnedFolders", "固定文件夹"),
            );
      ...
      if (pinnedFolders.length > 0) {
        const regularHeader = Utils.createElement(
          "h3",
          "section-header",
          {},
          I18n.getMessage("allFolders", "所有文件夹"),
        );
