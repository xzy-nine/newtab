...
export const DataSync = {
  ...
  handleSyncModeChange(mode) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("dataSyncSettingsChanged", {
  ...
  createSyncStatusDisplay() {
    ...
    const statusLabel = Utils.createElement(
      "span",
      "sync-status-label",
      {},
      I18n.getMessage("syncStatus", "状态") + ": ",
    );
    const statusValue = Utils.createElement("span", "sync-status-value", {
    ...
    const lastSyncLabel = Utils.createElement(
      "span",
      "sync-last-label",
      {},
  ...
  createSyncActionsPanel() {
    ...
    // 检查云端数据按钮
    const checkBtn = Utils.createElement(
      "button",
      "btn btn-info sync-check-btn",
      {},
    ...
    // 手动上传按钮
    const uploadBtn = Utils.createElement(
      "button",
      "btn btn-primary sync-upload-btn",
      {},
    ...
    const downloadBtn = Utils.createElement(
      "button",
      "btn btn-secondary sync-download-btn",
      {},
    ...
    // 清除云端数据按钮
    const clearBtn = Utils.createElement(
      "button",
      "btn btn-danger sync-clear-btn",
      {},
  ...
  async manualSync(mode) {
    try {
      ...
      // 触发同步状态更新事件
      window.dispatchEvent(
        new CustomEvent("syncStatusUpdated", {
          detail: {
            type: "syncComplete",
            mode: mode,
            timestamp: Date.now(),
  ...
  async checkCloudData() {
    try {
      ...
      if (keys.length === 0) {
        ...
        message =
          `${I18n.getMessage("cloudDataInfo", "云端数据信息")}:\n` +
