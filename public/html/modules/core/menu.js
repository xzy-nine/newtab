  ...
  showFormModal: (title, formItems, onConfirm, confirmText, cancelText, options = {}) => {
    ...
    formItems.forEach((item) => {
      ...
      if (item.value !== undefined) {
        ...
        } else if (item.type !== "select" && item.type !== "range") {
          // 选择框和range已在上面处理
          input.value = item.value;
        }
      }
  ...
  ImageSelector: {
    ...
    show: function (options = {}) {
      ...
      const modalContentClass =
        mode === "background" ? "modal-content modal-content-wide" : "modal-content";
      ...
      const cancelBtn = Utils.createElement(
        "button",
        "btn",
        { id: `${modalId}-cancel` },
        cancelText,
      );
      const confirmBtn = Utils.createElement(
        "button",
        "btn btn-primary",
        { id: `${modalId}-confirm` },
        confirmText,
