...
export const IconManager = {
  ...
  async getIconUrl(url, element = null) {
    ...
    this.fetchIconFromSources(url, domain).then((realIcon) => {
      if (
        realIcon &&
        this.stripFallbackPrefix(realIcon) !== this.stripFallbackPrefix(fallbackIcon)
      ) {
        if (
          element &&
          element.style.backgroundImage.includes(this.stripFallbackPrefix(fallbackIcon))
        ) {
  ...
  async fetchIconFromSources(url, domain, options = {}) {
    ...
    if (
      !ignoreFallbackCache &&
      domainCache[domain] &&
      domainCache[domain].startsWith(FALLBACK_ICON_PREFIX)
    ) {
  ...
  async setIconForElement(img, url) {
    ...
    try {
      ...
      const shouldRetry =
        (!hasCache || isFallbackCache) && retryCount < MAX_FALLBACK_RETRIES_PER_PAGE;
      if (shouldRetry) {
        fallbackRetryCounts.set(domain, retryCount + 1);
        this.fetchIconFromSources(url, domain, { ignoreFallbackCache: true }).then((realIcon) => {
          if (
            realIcon &&
            this.stripFallbackPrefix(realIcon) !== this.stripFallbackPrefix(iconData) &&
            img.isConnected &&
            img.dataset.originalUrl === url
          ) {
  ...
  async resetIcon(url) {
    try {
      ...
      const keysToRemove = [url, domain, urlObj.hostname, url.split("/").slice(0, 3).join("/")];

      await Promise.all(
        keysToRemove.map((key) => chrome.storage.local.remove(key).catch(() => {})),
      );
    } catch (error) {
    ...
    const extractKeyword = (domain) => {
      ...
      if (workingParts.length > 1 && tldPattern.test(workingParts[workingParts.length - 1])) {
        ...
        if (
          ...
          secondLevelPattern.test(workingParts[workingParts.length - 1])
        ) {
          workingParts.pop();
        }
      }
