...
export const Settings = {
    ...
    const categories = [
      {
        ...
        items: [...I18n.createSettingsItems(), ...ThemeManager.createSettingsItems()],
      },
      {
        id: "notifications",
        icon: "\uE700",
  ...
  showSettingsModal: () => {
    ...
    categories.forEach((category) => {
      const categoryItem = Utils.createElement(
        "div",
  ...
  createSettingItem: async (item) => {
    ...
    switch (item.type) {
      ...
      case "range":
        ...
        const rangeValue = Utils.createElement(
          "span",
          "range-value",
          {},
      ...
      case "select":
        ...
        item.options.forEach((option) => {
          const optionElement = Utils.createElement(
            "option",
            "",
            { value: option.value },
            option.label,
      ...
      case "radio":
        ...
        item.options.forEach((option) => {
          ...
          const radioLabel = Utils.createElement(
            "label",
            "",
            { for: `${item.id}-${option.value}` },
            option.label,
          );
              ...
              customControl.addEventListener("click", () => {
                ...
                if (Settings.unlockClicks >= 5) {
                  ...
                  Settings.getCategories().forEach((cat) => {
                    const catElem = Utils.createElement(
                      "div",
      ...
      case "button":
        const button = Utils.createElement(
          "button",
          `btn ${item.buttonClass || "btn-primary"}`,
          {
            id: item.id,
            type: "button",
          },
          ...
          try {
            ...
            if (widgets.length === 0) {
              container.innerHTML =
                ...
                I18n.getMessage("settingsWidgetsEmpty", "暂无可用小部件") +
                "</div>";
              return container;
            }
            ...
            widgets.forEach((widget) => {
              ...
              const widgetIcon = Utils.createElement(
                "span",
                "widget-list-icon",
                {},
                widget.icon || "\uE763",
              ...
              const widgetName = Utils.createElement(
                "div",
                "widget-list-name",
                {},
                widget.name || widget.type,
              );
              const widgetDesc = Utils.createElement(
                "div",
                "widget-list-desc",
                {},
                widget.description || "",
              );
              const widgetType = Utils.createElement(
                "div",
                "widget-list-type",
                {},
                widget.isCustom
                  ? I18n.getMessage("settingsWidgetsCustom", "自定义")
                  : I18n.getMessage("settingsWidgetsBuiltIn", "内置"),
              );
            ...
            container.innerHTML =
              '<div class="widget-list-error">' +
              I18n.getMessage("settingsWidgetsError", "获取小部件列表失败") +
              "</div>";
          }
    ...
    const formItems = [
      ...
      },
      {
        ...
        placeholder: I18n.getMessage(
          "settingsWidgetsJSPlaceholder",
          "小部件的JavaScript代码，必须包含initialize方法",
        ),
        value: "",
      },
    ];
