...
export const SearchEngineAPI = {
  ...
  async createEngineListControl() {
    ...
    try {
      ...
      const errorMsg = Utils.createElement(
        ...
        "error-message",
        {},
        I18n.getMessage("loadEngineListError", "加载搜索引擎列表失败"),
      );
  ...
  createSearchUI() {
    ...
    const searchEngineIcon = Utils.createElement("img", "", {
      ...
      alt: "Search Engine",
      style:
        "width:24px;height:24px;margin:0 10px 0 15px;cursor:pointer;object-fit:contain;opacity:0.7;transition:opacity 0.3s;",
    });
  ...
  renderSearchEngineSelector(firstRender = false) {
    ...
    searchEngines.forEach((engine, index) => {
      const menuItem = Utils.createElement(
        "div",
        "search-engine-item" + (index === currentEngineIndex ? " active" : ""),
      );
    ...
    // 添加自定义搜索引擎选项
    const addCustomEngine = Utils.createElement(
      "div",
      "search-engine-add",
      {},
  ...
  showAddSearchEngineModal() {
    ...
    Menu.showFormModal(
      ...
      (formData) => {
        ...
        SearchEngineAPI.addCustomEngine({ name, url, icon }).then((success) => {
          if (!success) {
            Notification.notify({
              title: I18n.getMessage("error", "错误"),
