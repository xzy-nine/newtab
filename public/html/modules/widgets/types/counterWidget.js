...
export default {
  ...
  adjustSize: function (container) {
    ...
    try {
      ...
      const needsHeightAdjustment =
        height < this.config.min.height || height > this.config.max.height;
        ...
        requestAnimationFrame(() => {
          if (needsWidthAdjustment) {
            const newWidth = Math.max(
              this.config.min.width,
              Math.min(this.config.max.width, width),
            );
            container.style.width = `${newWidth}px`;
          }
          if (needsHeightAdjustment) {
            const newHeight = Math.max(
              this.config.min.height,
              Math.min(this.config.max.height, height),
            );
  ...
  addEventListeners: function (container, content) {
    ...
    titleElement.addEventListener("dblclick", (e) => {
      ...
      const currentTitle =
        container.widgetData.title || I18n.getMessage("counterDefaultTitle", "计数");
    ...
    document.addEventListener("click", (e) => {
      ...
      if (titleElement.classList.contains("editing") && !titleElement.contains(e.target)) {
        // 获取输入框并模拟blur事件
