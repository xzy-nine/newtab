...
async function loadDirectTranslations() {
  try {
    ...
    try {
      ...
      directTranslations = response.ok ? await response.json() : {};
    } catch (error) {
...
async function showMobileInstruction(url) {
  try {
    ...
    const mobileMessage = await getDirectMessage(
      "mobileInstructionMessage",
      "请在浏览器设置中将新标签页设置为：{0}",
    );
    const desktopMessage = await getDirectMessage(
      "desktopInstructionMessage",
      "请在桌面浏览器设置中将本扩展设为新标签页",
    );

    return Notification.notify({
      title,
      message: isMobile ? mobileMessage.replace("{0}", finalUrl) : desktopMessage,
