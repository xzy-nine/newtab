...
export function normalizeSearchEngines(engines: unknown): SearchEngine[] {
  ...
  const valid = engines.filter((e): e is SearchEngine => 
...
export function normalizeAppSettings(value: unknown): AppSettings {
  ...
  return {
    ...
    currentEngineIndex: typeof candidate.currentEngineIndex === 'number' && 
      candidate.currentEngineIndex >= 0 
      ? candidate.currentEngineIndex 
      : DEFAULT_APP_SETTINGS.currentEngineIndex,
    language: ['zh', 'en'].includes(candidate.language || '') 
      ? candidate.language! 
      : DEFAULT_APP_SETTINGS.language,
    showBookmarks: typeof candidate.showBookmarks === 'boolean' 
      ? candidate.showBookmarks 
      : DEFAULT_APP_SETTINGS.showBookmarks,
    showClock: typeof candidate.showClock === 'boolean' 
      ? candidate.showClock 
      : DEFAULT_APP_SETTINGS.showClock,
    showWidgets: typeof candidate.showWidgets === 'boolean' 
      ? candidate.showWidgets 
      : DEFAULT_APP_SETTINGS.showWidgets,
    backgroundImageUrl: typeof candidate.backgroundImageUrl === 'string' 
      ? candidate.backgroundImageUrl 
      : DEFAULT_APP_SETTINGS.backgroundImageUrl,
    backgroundEnabled: typeof candidate.backgroundEnabled === 'boolean' 
